...
<?php
    ...
    $pagesql = "SELECT COUNT(*) as cnt from board_esc4 where $category like '%$keyword%'";
            ...
            <div class="board_contents_wrapper">
                <div class="table_search">
                    <form action="search_result.php" method="GET">
                        ...
                        <option value="title" <?php if ($category == 'title') {echo "selected";} ?>> 제목 </option>
                        <option value="content" <?php if ($category == 'content') {echo "selected";} ?>> 내용 </option>
                        <option value="name" <?php if ($category == 'name') {echo "selected";} ?>> 작성자 </option>
                        </select>
                    ...
                    <table class="table table-hover">
                        ...
                        <thead>
                            <tr>
                                <th> NO </th>
                                <th scope="col">
                                    <select name="search_category" id="" class="category">
                        ...
                        <tbody>
                            <?php
                                $sql = "SELECT * from board_esc4 where $category like '%$keyword%' order by idx desc limit $start_num, $list";
                ...
                <?php if($bno) { ?>
                    ...
                    <div class="pagination">
                        <ul>
                            <?php
                                if ($page > $block_ct) { 
                                    echo "<li> <a href='?search_category=$category&keyword=$keyword&page=1' class='button start'> <i class='fa-solid fa-angles-left'></i> </a> </li>";
                                if ($block_num > 1) {
                                    $prev_block_end = $block_start - 1; // 이전 블록의 마지막 페이지 번호
                                    $prev_block_start = ($block_num - 2) * $block_ct + 1; // 이전 블록의 첫 페이지 번호
                                
                                    echo "<li> <a href='?search_category=$category&keyword=$keyword&page=$prev_block_end' class='button previous'> <i class='fa-solid fa-angle-left'></i> </a> </li>";
                                }
                                }

                                for ($i = $block_start; $i <= $block_end; $i++) {
                                    if ($page == $i) {
                                        echo "<li> <a href='?search_category=$category&keyword=$keyword&page=$i' class='active'> $i </a> </li>";
                                    } else {
                                        echo "<li> <a href='?search_category=$category&keyword=$keyword&page=$i'> $i </a> </li>";
                                    }
                                }
                                ...
                                if ($page < $total_page) {
                                    if ($total_block > $block_num) {
                                        ...
                                        echo "<li> <a href='?search_category=$category&keyword=$keyword&page=$next_block_first_page' class='button next'> <i class='fa-solid fa-angle-right'> </i> </a> </li>";
                                    }
                                    echo "<li> <a href='?search_category=$category&keyword=$keyword&page=$total_page' class='button last'> <i class='fa-solid fa-angles-right'></i> </a> </li>";
                                }
                            ?>
                        </ul>
                    </div>
